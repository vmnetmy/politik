...
function EventCard({ event, registry }: { event: StateElectionEvent; registry: ConstituencyRegistry }) {
  ...
  return <Link className="prn-event-card" to={stateElectionPath(event.stateId, event.year)}>
...
export function StateElectionIndexPage() {
  ...
  return <>
    ...
    <aside className="storage-note prn-source-note"><Icon name="database" size={19}/><div><strong>Keputusan rasmi SPR</strong><p>Data Terbuka SPR, keputusan MySPR Semak semasa dan keputusan tertangguh N.42 Tioman digabungkan secara deterministik.</p></div></aside>
  </>;
}

function resolveEvent(results: StateElectionData, stateName?: string, year?: string) {
  const matching = results.events.filter((item) => item.stateId === stateName).sort((a, b) => b.electionDate.localeCompare(a.electionDate));
  return year ? matching.find((item) => item.year === Number(year)) : matching[0];
}
...
function DunResultCard({ event, contest, dun }: { event: StateElectionEvent; contest: StateElectionContest; dun: DunReference }) {
  ...
  return <Link className="prn-dun-card" to={stateDunResultPath(event.stateId, event.year, toSlug(dun.name))}>
...
export function StateElectionPage() {
  ...
  const event = value ? resolveEvent(value.results, stateName, year) : undefined;
...
export function StateDunResultPage() {
  const { stateName, year, dunName } = useParams();
  const { value, error } = useStateElectionData();
  if (error) return <ErrorStateElections message={error}/>;
  if (!value) return <LoadingStateElections/>;
  const event = resolveEvent(value.results, stateName, year);
  const state = value.constituencies.states.find((item) => item.id === event?.stateId);
  ...
  return <>
    ...
    <section className="route-hero prn-dun-hero"><div className="breadcrumbs"><Link to={PRN_BASE}>PRN</Link><span>/</span><Link to={stateElectionPath(state.id, event.year)}>{state.name} {event.year}</Link><span>/</span><strong>{dun.code}</strong></div><span className="overline">KEPUTUSAN PILIHAN RAYA NEGERI</span><h1>{dun.name}</h1><p>{dun.code} · {parliament.code} {parliament.name} · {formatDate(contest.electionDate)}</p></section>
    <section className="parliament-detail-grid prn-result-layout"><article className="panel candidate-detail"><div className="section-heading"><div><span className="eyebrow">KEPUTUSAN PENUH</span><h2>Semua calon</h2></div><span className="route-count">{formatNumber(contest.validVotes)} undi sah</span></div><div className="candidate-list">{ranked.map((candidate, candidateIndex) => <div className={`candidate-row ${candidate.status === "winner" ? "is-winner" : ""}`} key={candidate.id}><div className="candidate-rank">{String(candidateIndex + 1).padStart(2, "0")}</div><div className="candidate-copy"><div className="candidate-name-line"><strong>{candidate.name}</strong><span>{formatNumber(candidate.votes)}</span></div><div className="candidate-meta"><span className="candidate-identities"><TicketLogo name={candidate.party} shortName={candidate.shortName}/></span><span>{formatPct(candidate.share, 2)}</span></div><div className="result-track"><span style={{ width: `${candidate.share * 100}%`, background: ticketColor(candidate.shortName) }}/></div></div></div>)}</div></article>
      <aside className="detail-sidebar"><section className="winner-card prn-winner-card" style={{ "--winner": ticketColor(winner.shortName) } as React.CSSProperties}><div className="winner-label"><span>PEMENANG PRN</span><TicketLogo name={winner.party} shortName={winner.shortName}/></div><h3>{winner.name}</h3><div className="winner-stats"><div><strong>{formatNumber(winner.votes)}</strong><span>undi</span></div><div><strong>{formatPct(winner.share, 2)}</strong><span>bahagian undi</span></div><div><strong>{formatNumber(contest.majorityVotes)}</strong><span>majoriti</span></div></div></section><div className="detail-facts"><div><span>Pemilih berdaftar</span><strong>{formatOptionalNumber(contest.registeredVoters)}</strong></div><div><span>Keluar mengundi</span><strong>{formatOptionalPct(contest.turnoutPct)}</strong></div><div><span>Calon bertanding</span><strong>{contest.candidates.length}</strong></div><div><span>Undi ditolak</span><strong>{formatOptionalNumber(contest.rejectedVotes)}</strong></div></div><Link className="prn-geography-link" to={dunPath(state.id, toSlug(parliament.name), toSlug(dun.name))}><div><span>HIERARKI KAWASAN</span><strong>DUN → PDM → lokaliti</strong></div><Icon name="arrow" size={17}/></Link></aside>
    </section>
    <nav className="adjacent-seats" aria-label="DUN bersebelahan">{previous ? <Link to={stateDunResultPath(state.id, event.year, toSlug(previous.name))}><span>← SEBELUMNYA</span><strong>{previous.code} {previous.name}</strong></Link> : <i/>}{next && <Link to={stateDunResultPath(state.id, event.year, toSlug(next.name))}><span>SETERUSNYA →</span><strong>{next.code} {next.name}</strong></Link>}</nav>
  </>;
}
