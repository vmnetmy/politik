...
export function StateElectionEditionPage({ assemblyNumberOverride }: { assemblyNumberOverride?: string }) {
  ...
  const dateLabel = dates.length ? dates[0] === dates.at(-1) ? formatDate(dates[0]) : `${formatDate(dates[0])} – ${formatDate(dates.at(-1)!)}` : "";
  return <>
...
export function StateDunResultPage() {
  ...
  return <>
    ...
    <section className="route-hero prn-dun-hero"><div className="breadcrumbs"><Link to={PRN_BASE}>PRN</Link><span>/</span><Link to={stateElectionEditionPath(event.assemblyNumber)}>PRN-{event.assemblyNumber}</Link><span>/</span><Link to={stateElectionPath(state.id, event.assemblyNumber)}>{state.name}</Link><span>/</span><strong>{dun.code}</strong></div><span className="overline">KEPUTUSAN PILIHAN RAYA NEGERI</span><h1>{dun.name}</h1><p>{dun.code} · {parliament.code} {parliament.name} · {formatDate(contest.electionDate)}</p></section>
