...
export function ParliamentGeography({ seat, geography, constituencies }: { seat: ElectionData["seats"][number]; geography: GeographyData; constituencies: ConstituencyRegistry }) {
  ...
  const coveredPdms = pdms.filter((item) => item.localityIds.length > 0).length;
  ...
  return <section className="explorer-section parliament-geography"><div className="explorer-heading"><div><span className="eyebrow">PECAHAN KAWASAN</span><h2>{duns.length ? `${duns.length} DUN dalam ${seat.name}` : `Daerah mengundi ${seat.name}`}</h2><p>Hierarki rasmi SPR daripada Parlimen kepada {duns.length ? "DUN dan " : ""}daerah mengundi.</p></div><div className="result-count"><strong>{pdms.length}</strong><span>PDM</span></div></div>
    <CoverageStatus covered={coveredPdms} total={pdms.length} label="Lokaliti rasmi" status={coverageStatus(coveredPdms, pdms.length)}/>
...
function PdmCard({ pdm, stateName, parliamentName, dunName }: { pdm: GeographyPdm; stateName: string; parliamentName: string; dunName?: string }) {
  ...
  return <Link className="geography-card" to={paths.pdm(toSlug(stateName), toSlug(parliamentName), pdm.slug, dunName ? toSlug(dunName) : undefined)}>
    <div className="geography-card-code"><span>{pdm.code}</span><span className="geography-card-badges">{pdm.hasScoresheet && <b>SKOR RASMI</b>}<b className={pdm.localityIds.length ? "has-locality" : "no-locality"}>{pdm.localityIds.length ? `${pdm.localityIds.length} LOKALITI` : "LOKALITI BELUM ADA"}</b></span></div>
...
export function DunPage(props: GeographyProps) {
  ...
  return <>
    ...
    <section className="geography-summary"><article><span>DAERAH MENGUNDI</span><strong>{pdms.length}</strong></article><article><span>PEMILIH BERDAFTAR 2022</span><strong>{formatNumber(registered)}</strong></article><article><span>PDM DENGAN SCORESHEET</span><strong>{pdms.filter((item) => item.hasScoresheet).length}</strong></article><article><span>PDM DENGAN LOKALITI</span><strong>{coveredPdms}</strong></article></section>
...
export function PdmPage(props: GeographyProps) {
  ...
  return <>
    ...
    <section className="pdm-directory-grid"><article className="panel geography-list"><div className="section-heading"><div><span className="eyebrow">PUSAT MENGUNDI</span><h2>{centres.length ? `${centres.length} pusat` : "Belum diarkib"}</h2></div></div>{centres.map((centre) => <div key={centre.id}><strong>{centre.name}</strong><span>{centre.streamCount} saluran</span></div>)}{!centres.length && <p>Maklumat pusat akan muncul apabila scoresheet Parlimen ini tersedia.</p>}</article>
      <article className="panel geography-list"><div className="section-heading"><div><span className="eyebrow">LOKALITI RASMI</span><h2>{localities.length ? `${localities.length} lokaliti` : "Liputan belum tersedia"}</h2></div><CoverageStatus covered={localities.length ? 1 : 0} total={1} label="PDM" status={localities.length ? "complete" : "unavailable"}/></div>{localities.map((locality) => <Link key={locality.id} to={paths.locality(toSlug(state.name), toSlug(parliament.name), pdm.slug, locality.slug, dun ? toSlug(dun.name) : undefined)}><span>{locality.code}</span><strong>{locality.name}</strong><Icon name="arrow" size={15}/></Link>)}{!localities.length && <p>Tiada lokaliti direka atau disimpulkan. Ia hanya dipaparkan selepas sumber DPT rasmi diarkibkan.</p>}</article></section>
  </>;
}
...
export function LocalityPage(props: GeographyProps) {
  ...
  const latestSource = locality.sourceRefs.at(-1)!;
  return <>
    ...
    <section className="locality-panel panel"><span className="eyebrow">LOKALITI DALAM PDM {pdm.name}</span><h2>{locality.name}</h2><dl><div><dt>Kod lokaliti</dt><dd>{locality.code}</dd></div><div><dt>Daerah mengundi</dt><dd>{pdm.code} {pdm.name}</dd></div><div><dt>Sumber terkini</dt><dd>{latestSource.label}</dd></div><div><dt>Jejak sumber</dt><dd>{locality.firstSeenAt === locality.lastSeenAt ? locality.lastSeenAt : `${locality.firstSeenAt} – ${locality.lastSeenAt}`} · {locality.sourceRefs.length} dokumen</dd></div></dl><a href={latestSource.url} target="_blank" rel="noreferrer">Buka dokumen DPT rasmi SPR ↗</a><p>Maklumat lokaliti ini dikunci kepada jejak DPT rasmi sehingga {locality.lastSeenAt}; ia tidak digunakan untuk mengubah keputusan sejarah {props.data.metadata.shortTitle}.</p></section>
  </>;
}
