...
export function ScoresheetDetail({ seat, data, indexEntry }: { seat: Seat; data: ElectionData; indexEntry?: ScoresheetIndexEntry }) {
  ...
  return <section className="scoresheet-section">
    <div className="scoresheet-heading"><div><span className="eyebrow">HELAIAN MATA SPR 760</span><h2>Keputusan mengikut saluran</h2><p>{formatNumber(result.totals.pollingStreams)} saluran daripada {formatNumber(indexEntry.pollingDistrictCount)} kumpulan daerah mengundi. Data ini dimuatkan hanya untuk kerusi yang sedang dilihat.</p></div><span className="scoresheet-source-status is-authoritative">SUMBER RASMI</span></div>
