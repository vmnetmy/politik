...
export function ScoresheetDetail({ seat, data, indexEntry, pollingPlaces }: { seat: Seat; data: ElectionData; indexEntry?: ScoresheetIndexEntry; pollingPlaces: PollingPlacesData }) {
  ...
  if (!indexEntry) return <AsyncState kind="empty" title="Helaian mata terperinci belum tersedia" description={`Keputusan agregat ${edition.shortTitle} masih dipaparkan. Sumber mengikut daerah mengundi dan saluran belum ada dalam arkib untuk ${seat.code}.`} className="scoresheet-unavailable"/>;
  ...
  return <section className="scoresheet-section">
    ...
    <aside className="storage-note scoresheet-provenance"><Icon name="database" size={19}/><div><strong>{result.metadata.sourceFile}</strong><p>{isOfficialScoresheet ? "Dicetak" : "Tarikh data"} {result.metadata.printDate} · SHA-256 {result.metadata.sourceSha256}</p></div></aside>
  </section>;
}
